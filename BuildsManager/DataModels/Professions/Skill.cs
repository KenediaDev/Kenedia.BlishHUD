...
namespace Kenedia.Modules.BuildsManager.DataModels.Professions
{
    ...
    public class Skill : IDisposable, IBaseApiData
    {
        ...
        internal static Skill FromUShort(ushort id, ProfessionType profession, Services.Data data)
        {
            foreach (var race in data.Races.Items)
            {
            ...
            int skillid = 0;
            bool hasSkill = data.Professions?[profession]?.SkillsByPalette.TryGetValue((int)id, out skillid) == true;

            return hasSkill ? data.Professions?[profession]?.Skills[skillid] : null;
        }
        ...
        public static int GetRevPaletteId(int id)
        {
            ...
            {
                //Heal
                {4572, new(){ 26937, 27220, 27372, 28219, 28427, 29148, 45686, 62719, 62749, 77043}},
                //Util1
                {4614, new(){ 26821, 27322, 28379, 28516, 29209, 29310, 42949, 62832, 627020, 77243}},
                //Util2
                {4651, new(){ 26679, 27014, 27025, 27505, 28231, 29082, 40485, 62962, 62941, 77291}},
                //Util3
                {4564, new(){26557, 26644, 27107, 27715, 27917, 29197, 41220, 62878, 62796, 76805 }},
                //Elite
                {4554, new(){27356, 27760, 27975, 28287, 28406, 29114, 45773, 62942, 62687, 77001}},
            };
