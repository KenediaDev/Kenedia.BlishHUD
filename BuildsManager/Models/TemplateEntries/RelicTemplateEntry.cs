...
namespace Kenedia.Modules.BuildsManager.TemplateEntries
{
    public class RelicTemplateEntry : TemplateEntry
    {
        ...
        public override void FromCode(string code)
        {
            ...
            if (parts.Length == 1)
            {
                Item = int.TryParse(parts[0], out int id) ? BuildsManager.Data.Relics.Values.Where(e => e.MappedId == id).FirstOrDefault() : null;
            }
        }
        ...
        public override short[] GetFromCodeArray(short[] array)
        {
            ...
            Item = int.TryParse($"{array[0]}", out int id) ? BuildsManager.Data.Relics.Values.Where(e => e.MappedId == id).FirstOrDefault() : null;
