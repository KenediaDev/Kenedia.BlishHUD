...
namespace Kenedia.Modules.Characters.Controls
{
    public class TaskEntriesPanel : FlowPanel
    {
        ...
        private (Checkbox checkbox, CheckboxSyncState syncState) BuildHeader(TaskListModel selectedList)
        {
            ...
            int completedCount = selectedList.Entries.Count(e => e.Completed);
            int totalCount = selectedList.Entries.Count;
            ...
            int incompleteCount = selectedList.Entries.Count(e => !e.Completed);
            ...
            {
                Parent = headerPanel,
                Text = isLastIncomplete ? "Finish" : "Next",
                Width = 70,
                Height = 28,
            ...
            headerPanel.Resized += (s, e) =>
            {
                int pad = HeaderControlHorizontalPadding;
                int outerPad = pad + DragHandleWidth;
                int fixedWidth = outerPad + 20 + pad + entriesLabel.Width + pad + 70 + pad;
                if (hideButton.Visible)
                {
                    fixedWidth += pad + 165;
                }
                statusLabel.Width = Math.Max(0, headerPanel.Width - fixedWidth);
            };
