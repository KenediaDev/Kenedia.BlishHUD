...
using Race = Kenedia.Modules.AdvancedBuildsManager.DataModels.Race;
using Profession = Kenedia.Modules.AdvancedBuildsManager.DataModels.Professions.Profession;
using Skill = Kenedia.Modules.AdvancedBuildsManager.DataModels.Professions.Skill;
using Trait = Kenedia.Modules.AdvancedBuildsManager.DataModels.Professions.Trait;
using Specialization = Kenedia.Modules.AdvancedBuildsManager.DataModels.Professions.Specialization;
using Legend = Kenedia.Modules.AdvancedBuildsManager.DataModels.Professions.Legend;
using Kenedia.Modules.AdvancedBuildsManager.DataModels.Stats;
using Pet = Kenedia.Modules.AdvancedBuildsManager.DataModels.Professions.Pet;
    ...
    public class GW2API
    {
        ...
        public async Task GetItems(CancellationToken cancellation)
        {
            try
            {
                ...
                {
                    AdvancedBuildsManager.Logger.Info($"Saving {file} ...");
        ...
        public async Task GetSkillConnections(CancellationToken cancellation)
        {
            ...
            foreach (var skill in skills)
            {
                if (skill.Type != SkillType.Monster && skill.Professions.Count > 0)
                {
                    ...
                    AdvancedBuildsManager.Data.OldConnections.Add(skill.Id, connection);
                }
            }

            var cnts = AdvancedBuildsManager.Data.OldConnections.Values.ToList();
            foreach (var connection in AdvancedBuildsManager.Data.OldConnections)
            {
        ...
        public async Task FetchItems(CancellationToken cancellation)
        {
            ...
            {
                AdvancedBuildsManager.Logger.Warn($"RequestException {ex}");
            }
            catch (Exception ex)
            {
        ...
        public async Task CreateItemMap(CancellationToken cancellation)
        {
            try
            {
                ...
                AdvancedBuildsManager.Logger.Info($"Fetching a total of {raw_itemids.Count} items.");
                ...
                foreach (var ids in itemid_lists)
                {
                    if (ids != null)
                    {
                        ...
                        AdvancedBuildsManager.Logger.Info($"Fetching chunk {count}/{itemid_lists.Count}");
